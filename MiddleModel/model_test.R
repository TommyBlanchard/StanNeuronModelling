...
modelcode <- paste(readLines('beta_model2.stan'), collapse = '\n')
...
make_data <- function(betas,N_CELLS,DATA_PER_CELL){
  ...
  data <- list(N_CELLS=N_CELLS, 
               ...
               X = obs,
               xWishartPrior=array(c(10,.1)), # These should be calibrated to the scale of the noise
               yWishartPrior=array(c(.1,10))
  )      
  return(data);
}
